...
const Reviews = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(
        `${process.env.REACT_APP_API_URL}/api/rooms/${id}/review`,
        {
          rating,
          comment,
        },
  ...
  return (
    <div className="my-20">
      ...
      {reviews.length < 1 ? (
        <p className="text-red-600">There is no reviews yet</p>
      ) : (
        reviews.map((rev) => (
      ...
      <form onSubmit={handleSubmit}>
        <select
          ...
          className="p-2 rounded-tl-md rounded-tr-md sm:text-sm w-full border border-violet-500 outline-violet-500 my-3"
          name=""
          id=""
        >
        ...
        <textarea
          ...
          className="p-2 rounded-md sm:text-sm w-full border border-violet-500 outline-violet-500 my-3"
          required
        ></textarea>
        <button
          type="submit"
